...
(function() {
    ...
    class VitaProAdmin {
        ...
        showLoading($targetArea) {
            // Exibe um spinner de loading na área alvo (ou globalmente se não especificado)
            if (!$targetArea || !$targetArea.length) {
                $targetArea = jQuery('.vitapro-admin-wrapper, .wrap').first();
            }
            if ($targetArea.find('.vpa-admin-loading').length === 0) {
                $targetArea.append('<div class="vpa-admin-loading" style="position:absolute;top:50%;left:50%;transform:translate(-50%,-50%);z-index:9999;"><span class="dashicons dashicons-update spin" style="font-size:2.3em;color:#0057b8;"></span></div>');
                $targetArea.css('position', 'relative');
            }
        }

        hideLoading($targetArea) {
            if (!$targetArea || !$targetArea.length) {
                $targetArea = jQuery('.vitapro-admin-wrapper, .wrap').first();
            }
            $targetArea.find('.vpa-admin-loading').remove();
            $targetArea.css('position', '');
        }

        showSuccessMessage(message, $targetArea) {
            this.showToast(message, 'success', $targetArea);
        }

        showError(message, $targetArea) {
            this.showToast(message, 'error', $targetArea);
        }

        showToast(message, type = 'info', $targetArea) {
            // Exibe um toast na área alvo ou globalmente
            let $container = $targetArea && $targetArea.length ? $targetArea : jQuery('.vitapro-admin-wrapper, .wrap').first();
            let $toast = jQuery('<div class="vpa-admin-toast vpa-toast-' + type + '">' + message + '</div>');
            $container.append($toast);
            setTimeout(() => { $toast.fadeOut(400, function(){ jQuery(this).remove(); }); }, 3500);
        }

        // --- Exemplo de uso em operações AJAX ---
        saveSettings() {
            const $area = jQuery('.vpa-settings-content');
        ...
        // --- Otimização de upload/remoção de imagem nos metaboxes ---
        initServiceSettings() {
            // Duration field handling
            const durationFields = document.querySelectorAll('.vpa-service-duration');
            durationFields.forEach(field => {
